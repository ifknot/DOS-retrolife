...
namespace dos {
        ...
        public:
                ...
                *  @note maximum block size 64K is *not* checked requesting more results in undefined behaviour
                ...
                *  @param requested storage capacity in terms of number of type T
                */
                segmented_memory_block(size_type capacity) :
                        capacity_(capacity),
                        bytes_(capacity * sizeof(T)),
                        paragraphs_((bytes_ / PARAGRAPH_SIZE) + ((bytes_ & 0xF) ? 1 : 0)),  // ? need extra paragraph for any remainder
                ...
                /**
                * @breif return how many type T items can fit into the memory block
                */
                inline size_type capacity() {
                        return capacity_;
                }

                inline size_type bytes() {
                        return bytes_;
                }
                ...
                */
                ...
                void dump() {
                    for (int i = 0; i < bytes(); ++i) {
                        std::cout << *reinterpret_cast<char*>(data_.address + i);
                    }
                    std::cout << std::endl;
                }
