 ...
 *
 */
#ifndef HGA_GRAPHICS_UNPLOT_MULTI_POINT_H
#define HGA_GRAPHICS_UNPLOT_MULTI_POINT_H
...
namespace hga {
    ...
    namespace graphics {
	    ...
	    namespace screen_bound {
		    ...
		    inline void unplot_multi_point(const uint32_t* point_data, uint16_t size, uint8_t buffer = 0) {
                if (!size) return;
                __asm {
                    .8086
    #ifdef STACKING
